...
- **`<benchmark>/run_experiment.py`** — one training run per invocation
  (`--key {R|R1|LPA|FF} --order --panels --sigma --lr --nh --nn --trial --exp`).
  Writes `results/runs/<exp>/` with `run_*.json` (full config + final
...
- **`DeepONet/run_experiment_deeponet.py`** — PI-DeepONet runs with an explicit
  ...
  snapshots, and archived weights (`DeepONet/checkpoints/runs/`).
...
- **`experiments/`** — `run_queue.py` (resumable multi-process queue; jobs from
  `make_jobs*.py`), `aggregate_*.py` (tables/figures from the run JSONs; all
  paths resolve relative to the repository root), `plot_style.py` (figure
  conventions).
...
```bash
cd <benchmark>
python run_experiment.py --key LPA --order 6 --panels 30 --nh 2 --nn 10 --trial 0 --exp demo
```

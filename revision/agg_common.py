...
ROOT = '/Users/shr/Desktop/LPA'
V4 = os.path.join(ROOT, 'v4')
BENCH_DIRS = {
    'helmholtz2d': os.path.join(V4, '2D Helmholtz v2'),
    'diffusion_reaction': os.path.join(V4, 'Diffusion-Reaction'),
    'kovasznay': os.path.join(V4, 'Kovasznay flow'),
}
BENCH_LABELS = {
...
def load_runs(bench, exp):
    ...
    d = os.path.join(BENCH_DIRS[bench], 'results', 'revision', exp)
    rows = []
...
def load_acc_hist(bench, exp, nh, nn, key_id, trial):
    p = os.path.join(BENCH_DIRS[bench], 'results', 'revision', exp,

...
class Solver_PINN():
    ...
    def get_r(self, X_r):
        with tf.GradientTape(persistent=True) as tape2:
            ...
            u_x, v_x, p_x = map(lambda x:tf.squeeze(x,axis=-1), tf.split(tape1.batch_jacobian(pred,x), 3, axis=1))
            u_y, v_y, p_y = map(lambda x:tf.squeeze(x,axis=-1), tf.split(tape1.batch_jacobian(pred,y), 3, axis=1))
        ...
        x_mom = u * u_x + v * u_y + p_x - (1/Re)*(u_xx + u_yy)
        y_mom = u * v_x + v * v_y + p_y - (1/Re)*(v_xx + v_yy)
        contin = u_x + v_y

        # Gradient-enhancing: Residual gradients
        grads = {}
        for name, res in zip(['x_mom', 'y_mom', 'contin'], [x_mom, y_mom, contin]):
            res_x = tape2.gradient(res, x)
            res_y = tape2.gradient(res, y)
            grads[name + '_x'] = res_x
            grads[name + '_y'] = res_y
        del tape2

        return x_mom, y_mom, contin, grads

    def compute_loss(self):        
        # PDE Residual 및 gradient
        x_mom, y_mom, contin, grads = self.get_r(self.XY_r)

        # 원래 PDE loss
        Phi_r = tf.reduce_mean(tf.square(x_mom)) + \
                tf.reduce_mean(tf.square(y_mom)) + \
                tf.reduce_mean(tf.square(contin))

        # Gradient-enhancing term
        grad_loss = 0.
        for key, value in grads.items():
            if value is not None:
                grad_loss += tf.reduce_mean(tf.square(value))
        ...
        # 가중치 조정 (예: grad_weight=1e-1)
        grad_weight = 0
        total_loss = Phi_r + grad_weight * grad_loss + BX0 + BXL + BY0 + BYL
    ...
    def save_error(self):
        prediction = self.cur_pinn.model.predict(self.XY_test)
        u_pred, v_pred, p_pred = prediction[:,0], prediction[:,1], prediction[:,2]
        u_true, v_true, p_true = solution(self.XY_test)
        u_true = u_true.numpy().reshape(-1)
        v_true = v_true.numpy().reshape(-1)
        p_true = p_true.numpy().reshape(-1)

        l1_u = np.mean(np.abs(u_pred - u_true))
        l2_u = np.linalg.norm(u_pred - u_true, 2) / np.linalg.norm(u_true, 2)
        l1_v = np.mean(np.abs(v_pred - v_true))
        l2_v = np.linalg.norm(v_pred - v_true, 2) / np.linalg.norm(v_true, 2)
        l1_p = np.mean(np.abs(p_pred - p_true))
        l2_p = np.linalg.norm(p_pred - p_true, 2) / np.linalg.norm(p_true, 2)

        print(f'l1_absolute_error (u): {l1_u:.6e}, l2_relative_error (u): {l2_u:.6e}')
        print(f'l1_absolute_error (v): {l1_v:.6e}, l2_relative_error (v): {l2_v:.6e}')
        print(f'l1_absolute_error (p): {l1_p:.6e}, l2_relative_error (p): {l2_p:.6e}')

        np.savetxt(self.path + 'prediction_u_%s.txt' % self.cur_pinn.key, u_pred, delimiter=',')
        np.savetxt(self.path + 'exact_u_%s.txt' % self.cur_pinn.key, u_true, delimiter=',')
        np.savetxt(self.path + 'prediction_v_%s.txt' % self.cur_pinn.key, v_pred, delimiter=',')
        np.savetxt(self.path + 'exact_v_%s.txt' % self.cur_pinn.key, v_true, delimiter=',')
        np.savetxt(self.path + 'prediction_p_%s.txt' % self.cur_pinn.key, p_pred, delimiter=',')
        np.savetxt(self.path + 'exact_p_%s.txt' % self.cur_pinn.key, p_true, delimiter=',')

        with open(self.path2 + f'Error_{self.cur_pinn.num_hidden_layers}_{self.cur_pinn.num_neurons_per_layer}_{self.cur_pinn.key}.txt', 'w') as f:
            f.write('l1_absolute_error_u:  %s\n' % l1_u)
            f.write('l2_relative_error_u:  %s\n' % l2_u)
            f.write('l1_absolute_error_v:  %s\n' % l1_v)
            f.write('l2_relative_error_v:  %s\n' % l2_v)
            f.write('l1_absolute_error_p:  %s\n' % l1_p)
            f.write('l2_relative_error_p:  %s\n' % l2_p)